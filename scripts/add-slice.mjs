...
// Usage:
//   pnpm slice:add <name> [--dir=src/features] [--dry]
// Examples:
//   pnpm slice:add user
//   pnpm slice:add shoppingCart --dir=src/modules
//
...
async function main() {
  ...
  if (await pathExists(destDir)) {
    console.error(`❌ Slice directory already exists: ${destDir}`)
    process.exit(1)
  }

  const files = templatesFor(name)
  console.log(`\n🧩 Creating Redux slice "${kebab}" in ${destDir}`)
  if (opts.dry) {
